...
\textbf{(b) Prepare appropriate residual plots and comment on the model's adequacy.}
...
We observe our plots and see normality is good and residuals are patternless, meaning our model is adequate.
...
The p-values are presented on the ANOVA output. The same p-values were used to determine our test in part (a). p-values are listed: 1.09e-9, 1.65e-07, 0.018.

##5.5

\textbf{For the data in Problem 5.4, compute a 95 percent confidence interval estimate of the mean difference in response for feed rates of 0.20 and 0.25 in/min.}
...
We are 95% confident that the difference between feed rates 0.20 and 0.25 is (-31.45, -0.55).

##5.7

\textbf{Johnson and Leone (Statistics and Experimental Design in Engineering and the Physical Sciences, Wiley, 1977) describe an experiment to investigate warping of copper plates. The two factors studied were the temperature and the copper content of the plates. The response variable was a measure of the amount of warping. The data were as follows:}

\textbf{(a) Is there any indication that either factor affects the amount of warping? Is there any interaction between the factors? Use $\alpha$ =  0.05.}
...
\textbf{(c) Plot the average warping at each level of copper content and compare them to an appropriately scaled t distribution. Describe the differences in the effects of the different levels of copper content on warping. If low warping is desirable, what level of copper content would you specify?}

##5.8

\textbf{The factors that influence the breaking strength of a synthetic fiber are being studied. Four production machines and three operators are chosen and a factorial experiment is run using fiber from the same production batch. The results are as follows:}
...
From our F-tests, we find that temperature has an effect on warping, while copper content(%) does not.  In addition, there is no interaction effect on warping.
...
```
While our qqplot reveals that our model looks normally distributed, our residuals don't seem randomly distributed around 0, and the variances of residuals are not homogenous.
