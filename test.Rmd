...
A qqplot of our model, its residuals relative to the fit, and outputs to visualize variance homogeneity and their respective Bartlett Tests are shown below.

```{r}
# Baselines: Feed.Rate (.2), Depth (.15)
surface.lm=lm(Surface ~ factor(Feed.Rate)*factor(Depth),surface)
...
plot(fitted(surface.lm), res, ylim = c(-15,15))
abline(h = 0, lty = 2)
...
bartlett.test(Surface ~ factor(Feed.Rate), data = surface)
bartlett.test(Surface ~ factor(Depth), data = surface)
```

Our qqplot indicates that our model is normal.  Our plot of residuals relative to the fit has residuals randomly distributed and centered at zero, indicating the model is independent.  Our last two plots indicate that variances seem homogenous, but our Bartlett's Test indicates that variances in 'Feed.Rate' aren't homogenous.
